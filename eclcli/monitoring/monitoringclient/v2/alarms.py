...
class AlarmManager(base.Manager):
    ...
    def list(self, q=None, page=None, per_page=None):
        params = []
        if page:
            params.append('page=%s' % page)
        if per_page:
            params.append('per_page=%s' % per_page)
        return self._list(options.build_url(self._path(), q, params))
    ...
    def list_history(self, q=None, page=None, per_page=None):
        path = '/v2/alarm_histories'
        params = []
        if page:
            params.append('page=%s' % page)
        if per_page:
            params.append('per_page=%s' % per_page)
        url = options.build_url(path, q, params)
        return self._list(url, obj_class=AlarmChange)
