...
class Manager(base.HookableMixin):
    ...
    def _create(self, url, body, response_key, return_raw=False, **kwargs):
        ...
        # requirements from u1,
        # set admin_pass(only in response body of creation) to
        # return value of bearclient.create()
        try:
            admin_pass = None
            resp_content = getattr(_resp, '_content', '')
            if resp_content:
        ...
        with self.completion_cache('human_id', self.resource_class, mode="a"):
            with self.completion_cache('uuid', self.resource_class, mode="a"):
                rsc_cls = self.resource_class(self, body[response_key])
                if admin_pass:
                    setattr(rsc_cls, 'admin_pass', admin_pass)
                return rsc_cls
