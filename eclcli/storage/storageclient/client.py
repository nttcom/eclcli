...
class SessionClient(adapter.LegacyJsonAdapter):
    ...
    def request(self, url, method, **kwargs):
        ...
        if (parts[1] in _VALID_VERSIONS and parts[2] == ''
                and project_id is not None):
            url = '{0}{1}{2}'.format(endpoint, project_id, url)
        elif (endpoint.find(DEFAULT_VERSION) == -1
                and project_id is not None):
            endpoint = '{0}/{1}/{2}'.format(
                endpoint, DEFAULT_VERSION, project_id)
            url = '{0}{1}'.format(endpoint, url)

        # Note(tpatil): The standard call raises errors from
        # keystoneclient, here we need to raise the storageclient errors.
    ...
    def get_volume_api_version_from_endpoint(self):
        endpoint = self.get_endpoint()
        if not endpoint:
            msg = _('The Storage server does not support %s. Check your '
                    'providers supported versions and try again with '
                    'setting --os-share-api-version or the environment '
                    'variable OS_SHARE_API_VERSION.') % self.version
...
class HTTPClient(object):
    ...
    USER_AGENT = 'python-storageclient'
    ...
    def request(self, url, method, **kwargs):
        ...
        # SG-816 forcibly add content-type header depending on storage sdp spec.
    ...
    def authenticate(self):
        ...
        if self.version == "v2.0":
            ...
            # In some configurations storage makes redirection to
        ...
        if self.bypass_url:
            ...
            raise exceptions.Unauthorized('Storage Client')
...
def _construct_http_client(username=None, password=None, project_id=None,
    ...
    if session and not auth_plugin:
        kwargs.setdefault('user_agent', 'python-storageclient')
